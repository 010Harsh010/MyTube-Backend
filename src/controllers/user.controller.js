...
<div class="footer">
&copy; 2024 MyTube Inc. All rights reserved.
</div>
</div>
</body>
</html>
`;
    ...
    const encodedMessage = base64_encode(emailContent).replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=+$/, '');;
...
const verifyEmail = asynHandler(async (req, res) => {
  try {
    ...
    res.status(200).json(new ApiResponse(200,{ message: "User Updated Successfully" },"Updated Successfully"));
  } catch (error) {
    res.status(500).json(new ApiResponse(500, { message: "Internal Server Error"},"No update"))
  }
});
